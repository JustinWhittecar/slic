...
func (h *MechHandlerSQLite) List(w http.ResponseWriter, r *http.Request) {
	...
	if v := r.URL.Query().Get("armor_type"); v != "" {
		query += " AND vs.armor_type LIKE ?"
		args = append(args, "%"+v+"%")
	}
	if v := r.URL.Query().Get("structure_type"); v != "" {
		query += " AND vs.structure_type LIKE ?"
		args = append(args, "%"+v+"%")
	}
	if v := r.URL.Query().Get("equipment"); v != "" {
		...
		for _, name := range names {
			...
			if name != "" {
				query += ` AND v.id IN (SELECT ve.variant_id FROM variant_equipment ve JOIN equipment e ON e.id = ve.equipment_id WHERE e.name = ?)`
